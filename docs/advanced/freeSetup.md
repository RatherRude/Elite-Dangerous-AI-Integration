...
After installing Ollama, you need to download a model according to the instructions on their website. At the time of writing we recommend using `lucaelin/ministral-3-3b-cn-gguf`.
...
Load the model into LMStudio according to the instructions on their website. At the time of writing we recommend using `lucaelin/ministral-3-3b-cn-gguf`.
...
### 4.1 Using Edge-TTS (cloud-based)
We recommend using Edge-TTS as it is free and has good latency and quality.

### 4.2 Using Supertonic plugin (local, recommended)
...
- Slightly lower accuracy and missing words, low emotional expressiveness
