...
export class SettingsMenuComponent implements OnInit, OnDestroy {
  ...
  async onEventConfigChange(section: string, event: string, enabled: boolean) {
    ...
    if (activeChar && activeIndex >= 0) {
      try {
        ...
        this.snackBar.open(`Event setting saved for ${activeChar.name}`, "OK", {
          duration: 2000,
        });
      } catch (error) {
  ...
  loadSettingsFromConfig(config: Config): void {
    ...
    if (activeChar) {
      ...
      if (activeChar.personality_knowledge_scifi !== undefined) {
        this.settings.knowledge.scifi = activeChar.personality_knowledge_scifi;
      }
    ...
    if (!this.editMode) {
      ...
      if (this.selectedCharacterIndex >= 0 && this.config.characters) {
        this.localCharacterCopy = JSON.parse(
          JSON.stringify(this.config.characters[this.selectedCharacterIndex]),
        );
      }
      this.editMode = true;
    } else {
  ...
  private areCharactersEqual(char1: any, char2: any): boolean {
    ...
    return (
      ...
      char1.personality_ethical_alignment ===
        char2.personality_ethical_alignment &&
      char1.personality_moral_alignment === char2.personality_moral_alignment &&
      char1.personality_tone === char2.personality_tone &&
      char1.personality_character_inspiration ===
        char2.personality_character_inspiration &&
      char1.personality_language === char2.personality_language &&
      char1.personality_knowledge_pop_culture ===
        char2.personality_knowledge_pop_culture &&
      char1.personality_knowledge_scifi === char2.personality_knowledge_scifi &&
      char1.personality_knowledge_history ===
        char2.personality_knowledge_history &&
  ...
  addNewCharacter(): void {
    ...
    this.configService.changeConfig({ characters: this.config.characters })
      .then(() => {
        ...
        this.snackBar.open(`Character "${newCharacter.name}" created`, "OK", {
          duration: 3000,
        });
      }).catch((error) => {
