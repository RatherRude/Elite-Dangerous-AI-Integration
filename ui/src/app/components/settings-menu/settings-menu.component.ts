...
export class SettingsMenuComponent implements OnInit, OnDestroy {
  ...
  private inputDeviceNamesSubscription?: Subscription;
  ...
  ngOnInit() {
    ...
    this.inputDeviceNamesSubscription = this.configService.inputDeviceNames$
      .subscribe(
        (deviceNames) => {
          this.inputDeviceNames = deviceNames;
        },
      );
  }
  ...
  ngOnDestroy() {
    ...
    if (this.inputDeviceNamesSubscription) {
      this.inputDeviceNamesSubscription.unsubscribe();
    }
  }
  ...
  async onConfigChange(partialConfig: Partial<Config>) {
    ...
    if (this.config) {
      await this.configService.updateConfig(partialConfig);
    }
  }
  async onEventConfigChange(section: string, event: string, enabled: boolean) {
    if (this.config) {
      ...
      await this.configService.updateEventConfig(section, event, enabled);
    }
  }
