export const GameEventCategories: Record<string, string[]> = {
    "System": [
        // "Cargo",
        // "ClearSavedGame",
        "LoadGame",
        "Shutdown",
        "NewCommander",
        // "Materials",
        // "Missions",
        // "Progress",
        // "Powerplay",
        // "Rank",
        // "Reputation",
        // "Statistics",
        // "SquadronStartup",
        // "EngineerProgress",
    ],
    "Combat": [
        "Died",
        "Resurrect",
        "InDanger",
        "OutofDanger",
        "CombatEntered",
        "CombatExited",
        "LegalStateChanged",
        "CommitCrime",
        "Bounty",
        "CapShipBond",
        "Interdiction",
        "Interdicted",
        "BeingInterdicted",
        "EscapeInterdiction",
        "FactionKillBond",
        "FighterDestroyed",
        "HeatDamage",
        "HeatWarning",
        "HullDamage",
        "PVPKill",
        "ShieldState",
        "ShipTargeted",
        "BountyScanned",
        "UnderAttack",
        "CockpitBreached",
        "CrimeVictim",
        "SystemsShutdown",
        "SelfDestruct",
    ],
    "Trading": [
        "BuyTradeData",
        "CollectCargo",
        "EjectCargo",
        "MarketBuy",
        "MarketSell",
        "CargoTransfer",
        "Market",
    ],
    "Mining": [
        "AsteroidCracked",
        "MiningRefined",
        "ProspectedAsteroid",
        "LaunchDrone",
        "RememberLimpets",
    ],
    "Ship Updates": [
        "FSDJump",
        "FSDTarget",
        "StartJump",
        "FsdCharging",
        "SupercruiseEntry",
        "SupercruiseExit",
        "ApproachSettlement",
        "GlideModeExited",
        "GlideModeEntered",
        "Docked",
        "Undocked",
        "DockingCanceled",
        "DockingDenied",
        "DockingGranted",
        "DockingRequested",
        "DockingTimeout",
        "DockingComputerDocking",
        "DockingComputerUndocking",
        "DockingComputerDeactivated",
        "InDockingRange",
        "NavRoute",
        "NavRouteClear",
        "CrewLaunchFighter",
        "VehicleSwitch",
        "LaunchFighter",
        "DockFighter",
        "FighterRebuilt",
        "FuelScoop",
        "RebootRepair",
        "RepairDrone",
        "AfmuRepairs",
        "ModuleInfo",
        "Synthesis",
        "JetConeBoost",
        "JetConeDamage",
        "LandingGearUp",
        "LandingGearDown",
        "HudSwitchedToAnalysisMode",
        "HudSwitchedToCombatMode",
        "FlightAssistOn",
        "FlightAssistOff",
        "HardpointsRetracted",
        "HardpointsDeployed",
        "LightsOff",
        "LightsOn",
        "CargoScoopRetracted",
        "CargoScoopDeployed",
        "SilentRunningOff",
        "SilentRunningOn",
        "FuelScoopStarted",
        "FuelScoopEnded",
        "FsdMassLockEscaped",
        "FsdMassLocked",
        "LowFuelWarningCleared",
        "LowFuelWarning",
        "HighGravityWarning",
        "NightVisionOff",
        "NightVisionOn",
        "SupercruiseDestinationDrop",
    ],

    "SRV Updates": [
        "LaunchSRV",
        "DockSRV",
        "SRVDestroyed",
        "SrvHandbrakeOff",
        "SrvHandbrakeOn",
        "SrvTurretViewConnected",
        "SrvTurretViewDisconnected",
        "SrvDriveAssistOff",
        "SrvDriveAssistOn",
    ],
    "On-Foot Updates": [
        "Disembark",
        "Embark",
        "BookDropship",
        "BookTaxi",
        "CancelDropship",
        "CancelTaxi",
        "CollectItems",
        "DropItems",
        "BackpackChange",
        "BuyMicroResources",
        "SellMicroResources",
        "TransferMicroResources",
        "TradeMicroResources",
        "BuySuit",
        "BuyWeapon",
        "SellWeapon",
        "UpgradeSuit",
        "UpgradeWeapon",
        "CreateSuitLoadout",
        "DeleteSuitLoadout",
        "RenameSuitLoadout",
        "SwitchSuitLoadout",
        "WeaponSelected",
        "UseConsumable",
        "FCMaterials",
        "ScanOrganic",
        "SellOrganicData",
        "LowOxygenWarningCleared",
        "LowOxygenWarning",
        "LowHealthWarningCleared",
        "LowHealthWarning",
        "BreathableAtmosphereExited",
        "BreathableAtmosphereEntered",
        "DropShipDeploy",
    ],
    "Stations": [
        "MissionAbandoned",
        "MissionAccepted",
        "MissionCompleted",
        "MissionFailed",
        "MissionRedirected",
        "StationServices",
        "ShipyardBuy",
        "ShipyardNew",
        "ShipyardSell",
        "ShipyardTransfer",
        "ShipyardSwap",
        "StoredShips",
        "ModuleBuy",
        "ModuleRetrieve",
        "ModuleSell",
        "ModuleSellRemote",
        "ModuleStore",
        "ModuleSwap",
        "LoadoutEquipModule",
        "LoadoutRemoveModule",
        "Outfitting",
        "BuyAmmo",
        "BuyDrones",
        "RefuelAll",
        "RefuelPartial",
        "Repair",
        "RepairAll",
        "RestockVehicle",
        "FetchRemoteModule",
        "MassModuleStore",
        "ClearImpound",
        "CargoDepot",
        "CommunityGoal",
        "CommunityGoalDiscard",
        "CommunityGoalJoin",
        "CommunityGoalReward",
        "EngineerContribution",
        "EngineerCraft",
        "EngineerLegacyConvert",
        "MaterialTrade",
        "TechnologyBroker",
        "PayBounties",
        "PayFines",
        "PayLegacyFines",
        "RedeemVoucher",
        "ScientificResearch",
        "Shipyard",
        "CarrierJump",
        "CarrierBuy",
        "CarrierStats",
        "CarrierJumpRequest",
        "CarrierDecommission",
        "CarrierCancelDecommission",
        "CarrierBankTransfer",
        "CarrierDepositFuel",
        "CarrierCrewServices",
        "CarrierFinance",
        "CarrierShipPack",
        "CarrierModulePack",
        "CarrierTradeOrder",
        "CarrierDockingPermission",
        "CarrierNameChanged",
        "CarrierJumpCancelled",
        "ColonisationConstructionDepot",
        "FetchRemoteModuleCompleted",
        "ShipyardTransferCompleted",
    ],
    "Social": [
        "Idle",
        "CrewAssign",
        "CrewFire",
        "CrewHire",
        "ChangeCrewRole",
        "CrewMemberJoins",
        "CrewMemberQuits",
        "CrewMemberRoleChange",
        "EndCrewSession",
        "JoinACrew",
        "KickCrewMember",
        "QuitACrew",
        "NpcCrewRank",
        "Promotion",
        "Friends",
        "WingAdd",
        "WingInvite",
        "WingJoin",
        "WingLeave",
        "SendText",
        "ReceiveTextChannelLocal",
        "ReceiveTextChannelStarSystem",
        "ReceiveTextChannelSquadron",
        "ReceiveTextStation",
        "ReceiveTextPolice",
        "ReceiveTextDockingChatter",
        "ReceiveTextMilitary",
        "ReceiveTextCruiseLiner",
        "ReceiveTextCommuter",
        "ReceiveTextAX",
        "ReceiveTextTrader",
        "ReceiveTextPirate",
        "ReceiveTextPowers",
        "ReceiveTextMiner",
        "ReceiveTextExplorer",
        "ReceiveTextSmuggler",
        "ReceiveTextPassengerLiner",
        "ReceiveTextEscort",
        "ReceiveTextHitman",
        "ReceiveTextPropagandist",
        "ReceiveTextRescuer",
        "ReceiveTextConvoy",
        "ReceiveTextRefugeeFlotillaWar",
        "ReceiveTextProtester",
        "ReceiveTextOther",
        "AppliedToSquadron",
        "DisbandedSquadron",
        "InvitedToSquadron",
        "JoinedSquadron",
        "KickedFromSquadron",
        "LeftSquadron",
        "SharedBookmarkToSquadron",
        "SquadronCreated",
        "SquadronDemotion",
        "SquadronPromotion",
        "WonATrophyForSquadron",
        "PowerplayCollect",
        "PowerplayDefect",
        "PowerplayDeliver",
        "PowerplayFastTrack",
        "PowerplayJoin",
        "PowerplayLeave",
        "PowerplaySalary",
        "PowerplayVote",
        "PowerplayVoucher",
    ],
    "Exploration": [
        "CodexEntry",
        "DiscoveryScan",
        "Scan",
        "FSSAllBodiesFound",
        "FSSBodySignals",
        "FSSDiscoveryScan",
        "FirstPlayerSystemDiscovered",
        "FleetCarrierDiscovered",
        "ResourceExtractionDiscovered",
        "InstallationDiscovered",
        "NavBeaconDiscovered",
        "TouristBeaconDiscovered",
        "MegashipDiscovered",
        "GenericDiscovered",
        "OutpostDiscovered",
        "CombatDiscovered",
        "StationDiscovered",
        "UnknownSignalDiscovered",
        "MaterialCollected",
        "MaterialDiscarded",
        "MaterialDiscovered",
        "MultiSellExplorationData",
        "NavBeaconScan",
        "BuyExplorationData",
        "SAAScanComplete",
        "SAASignalsFound",
        "ScanBaryCentre",
        "SellExplorationData",
        "Screenshot",
        "ApproachBody",
        "LeaveBody",
        "Liftoff",
        "Touchdown",
        "DatalinkScan",
        "DatalinkVoucher",
        "DataScanned",
        "Scanned",
        "USSDrop",
        "NoScoopableStars",
    ],
};
